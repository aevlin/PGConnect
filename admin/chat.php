...
try {
    $stmt = $pdo->query('SELECT c.id,
        ...
        (SELECT COUNT(*) FROM messages m WHERE m.conversation_id = c.id AND m.sender_role IN ("user","owner") AND COALESCE(m.is_read_admin,0) = 0) AS unread_count
...
if ($activeId) {
    ...
    $pdo->prepare("UPDATE messages SET is_read_admin = 1 WHERE conversation_id = ? AND sender_role IN ('user','owner')")->execute([$activeId]);
}
?>
              ...
              <?php foreach ($conversations as $c): ?>
                <a class="list-group-item list-group-item-action <?php echo ($activeId == $c['id']) ? 'active' : ''; ?>" href="chat.php?c=<?php echo (int)$c['id']; ?>">
                  ...
                  <div class="small d-flex justify-content-between">
                    <span><?php echo htmlspecialchars($c['user_name']); ?> -> <?php echo htmlspecialchars($c['owner_name']); ?></span>
                    <?php if (!empty($c['unread_count'])): ?><span class="badge bg-danger"><?php echo (int)$c['unread_count']; ?></span><?php endif; ?>
                  </div>
                </a>
              <?php endforeach; ?>
            </div>
      ...
      <div class="col-lg-8">
        <div class="card border-0 shadow-sm p-3 h-100">
          ...
          <form method="POST" action="send-chat.php" class="row g-2">
            ...
            <div class="col-md-7">
              <input type="text" name="message" class="form-control" placeholder="Type admin message">
            </div>
            <div class="col-md-2">

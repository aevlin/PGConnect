...
if (!in_array($b['status'], ['requested','owner_approved','owner_rejected','payment_pending'], true)) {
    header('Location: owner-bookings.php'); exit;
}
...
if ($action === 'approve' || $action === 'reject') {
    ...
    $upd = $pdo->prepare("UPDATE bookings SET visit_note = CONCAT(COALESCE(visit_note,''), ' | Visit accepted'), owner_action_at = NOW() WHERE id = ?");
    $upd->execute([$id]);
    ...
    $upd = $pdo->prepare("UPDATE bookings SET visit_requested = 0, visit_note = CONCAT(COALESCE(visit_note,''), ' | Visit cancelled by owner'), owner_action_at = NOW() WHERE id = ?");
    $upd->execute([$id]);
    ...
    if ($ts !== false) {
        ...
        $upd = $pdo->prepare("UPDATE bookings SET visit_requested = 1, visit_datetime = ?, visit_note = CONCAT(COALESCE(visit_note,''), ' | Rescheduled by owner'), owner_action_at = NOW() WHERE id = ?");
